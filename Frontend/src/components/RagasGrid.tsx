...
export default function RagasGrid({ authUserId }) {
  ...
  const handleAddRow = async (e) => {
    if (newRaga.name == "" || newRaga.arohanam == "" || newRaga.avarohanam == "") {
  ...
  const processRowUpdate = async (updatedRow, originalRow) => {
    try {
      await axios.put("http://localhost:3000/ragas", updatedRow);
    } catch (err) {
      console.log(err);
    }
    setRefreshRagas(!refreshRagas);
  ...
  const handleDeleteRow = async () => {
    ...
    try {
      await axios
        .delete("http://localhost:3000/ragas", {
          params: {
            ids: selectedRowsId,
          },
        })
